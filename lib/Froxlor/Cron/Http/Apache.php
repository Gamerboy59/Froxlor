	...
	public function createVirtualHosts()
	{
		...
		foreach ($domains as $domain) {
			...
			if (($domain['deactivated'] == '1' || $domain['customer_deactivated'] == '1') && empty($ddr)) {
				...
				if(Settings::Get('webcache.activated')){
					$this->virtualhosts_data[$vhosts_filename] .= (new Varnish)->createVhostsVarnish($domain);
				} else {
...
class Varnish extends HttpConfigBase
{
	...
	public function createVhostsVarnish($domain){
		...
		$vhost_content .= $this->getWebroot($domain);
		if ($this->deactivated == false) {
			$vhost_content .= $this->composePhpOptions($domain, false);
			$vhost_content .= $this->getStats($domain);
		}
		$vhost_content .= $this->getLogfiles($domain);
		...
		if ($this->deactivated == false) {
			if ($domain['specialsettings'] != '') {
				$vhost_content .= $this->processSpecialConfigTemplate($domain['specialsettings'], $domain, $domain['ip'], $domain['port'], false) . "\n";
			}

			if (Settings::Get('system.default_vhostconf') != '') {
				$vhost_content .= $this->processSpecialConfigTemplate(Settings::Get('system.default_vhostconf'), $domain, $domain['ip'], $domain['port'], false) . "\n";
			}
		}
	    $vhost_content .= '</VirtualHost>' . "\n";
...
EOD;
		if (!file_exists($varnish_systemd_service_dir)) {
			FroxlorLogger::getInstanceOf()->logAction(FroxlorLogger::CRON_ACTION, LOG_NOTICE, 'webcache::createVarnishSystemdService: mkdir ' . escapeshellarg($varnish_systemd_service_dir));
			FileDir::safe_exec('mkdir ' . escapeshellarg($varnish_systemd_service_dir));
		}
		$varnish_systemd_service_data = '# ' . basename($varnish_systemd_service_file) . "\n" . '# Created ' . date('d.m.Y H:i') . "\n" . '# Do NOT manually edit this file, all changes will be deleted after the next domain change at the panel.' . "\n" . "\n" . $varnish_systemd_service_data;
		$varnish_systemd_service_file_handler = fopen($varnish_systemd_service_file, 'w');
		fwrite($varnish_systemd_service_file_handler, $varnish_systemd_service_data);
		fclose($varnish_systemd_service_file_handler);
...
EOD;
		...
		if (!file_exists($varnish_config_dir)) {
			FroxlorLogger::getInstanceOf()->logAction(FroxlorLogger::CRON_ACTION, LOG_NOTICE, 'webcache::createVarnishConfig: mkdir ' . escapeshellarg($varnish_config_dir));
			FileDir::safe_exec('mkdir ' . escapeshellarg($varnish_config_dir));
		}
		$varnish_config_data = '# ' . basename($varnish_config_file) . "\n" . '# Created ' . date('d.m.Y H:i') . "\n" . '# Do NOT manually edit this file, all changes will be deleted after the next domain change at the panel.' . "\n" . "\n" . $varnish_config_data;
		$varnish_config_file_handler = fopen($varnish_config_file, 'w');
		fwrite($varnish_config_file_handler, $varnish_config_data);
		fclose($varnish_config_file_handler);

		FroxlorLogger::getInstanceOf()->logAction(FroxlorLogger::CRON_ACTION, LOG_INFO, 'webcache::createVarnishConfig: restarting varnish');
...
EOD;
		...
		if (!file_exists($hitch_config_dir)) {
			FroxlorLogger::getInstanceOf()->logAction(FroxlorLogger::CRON_ACTION, LOG_NOTICE, 'webcache::createHitchConfig: mkdir ' . escapeshellarg($hitch_config_dir));
			FileDir::safe_exec('mkdir ' . escapeshellarg($hitch_config_dir));
		}
		$hitch_config_data = '# ' . basename($hitch_config_file) . "\n" . '# Created ' . date('d.m.Y H:i') . "\n" . '# Do NOT manually edit this file, all changes will be deleted after the next domain change at the panel.' . "\n" . "\n" . $hitch_config_data;
		$hitch_config_file_handler = fopen($hitch_config_file, 'w');
		fwrite($hitch_config_file_handler, $hitch_config_data);
		fclose($hitch_config_file_handler);
